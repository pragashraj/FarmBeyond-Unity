...
public class CowController : MonoBehaviour
{
    ...
    [SerializeField] private int maxAnimals = 3;
    ...
    private void SetPrefabs(int i)
    {
        ...
        string anim = rand == 0 ? "idle" : "pawL";
        instantiatedObject.GetComponent<Animation>().Play(anim);
        instantiatedObject.name = "Cow " + i.ToString();
    }
    ...
    private void HandleFeedOnClick()
    {
        ...
        int cows = gameManager.Cows;
        if (cows > 0)
        {
            int totalCornsNeeded = cows * cornsPerAnimal;
        ...
        {
            uIManager.SetWarnUI("You dont have cows to feed, purchase first");
        }
    }
    ...
    private void HandleCollectOnClick()
    {
        if (isCollectible)
        {
            ...
            isCollectible = false;
            isProcessing = false;
            int total = collectiblePerAnimal * gameManager.Cows;
            gameManager.CowMilks += total;
    ...
    {
        int animals = gameManager.Cows;
        if (maxAnimals == animals)
        {
            uIManager.SetWarnUI("Cows limit exceeded");
        }
        else
        {
