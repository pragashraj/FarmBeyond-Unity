...
public class PigManager : MonoBehaviour
{
    ...
    [SerializeField] private int cornsPerAnimal = 10;
    [SerializeField] private int galleonsPerAnimal = 10;
    [SerializeField] private int collectiblePerAnimal = 2;
    ...
    private void SetPrefabs(int i)
    {
        ...
        instantiatedObject.name = "Pig " + i.ToString();
    }
    ...
    private void HandleFeedOnClick()
    {
        ...
        int pigs = gameManager.Pigs;
        if (pigs > 0)
        {
            int totalCornsNeeded = pigs * cornsPerAnimal;
        ...
        {
            uIManager.SetWarnUI("You dont have pigs to feed, purchase first");
        }
    }
    ...
    private void HandleCollectOnClick()
    {
        if (isCollectible)
        {
            ...
            isCollectible = false;
            isProcessing = false;
            int total = collectiblePerAnimal * gameManager.Pigs;
            gameManager.PigMeats += total;
    ...
    {
        int animals = gameManager.Pigs;
        if (maxAnimals == animals)
        {
            uIManager.SetWarnUI("Pigs limit exceeded");
        }
        else
        {
